    ...
    def parse(example):
        ...
        features = tf.parse_single_example(example,
                                           features={
                                               ...
                                               "alternative_idxs": tf.FixedLenFeature([], tf.string),
                                               "answer": tf.FixedLenFeature([], tf.int64),
                                               "id": tf.FixedLenFeature([], tf.int64),
                                               "nlp_feature": tf.FixedLenFeature([187], tf.float32)
                                           })
        ...
        alternative_idxs = tf.reshape(tf.decode_raw(
            features["alternative_idxs"], tf.int32), [3, ans_limit])
        answer = features["answer"]
        qa_id = features["id"]
        nlp_feature= features["nlp_feature"]
        return passage_idxs, question_idxs, alternative_idxs, answer, qa_id ,nlp_feature
    return parse
