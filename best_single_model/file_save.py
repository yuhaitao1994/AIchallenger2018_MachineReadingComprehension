...
def save_dev(config):
    ...
    with tf.Session(graph=g1, config=sess_config) as sess1:
        with g1.as_default():
            ...
            # 需要手动更改路径
            saver1.restore(
                sess1, "./log/modellalala/model_99000_devAcc_0.751301.ckpt")
    ...
    with open("./dev_soft/nlp_model_0.7513.txt", "wb") as f1:  # 手动更改保存的名字，路径不用改
...
def save_test(config):
    ...
    with open("./test_soft/nlp_model_0.7446.txt", "wb") as f1:
        pickle.dump(predict_dict, f1)
