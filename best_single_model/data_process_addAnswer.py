...
data_process_addAnswer.py：数据预处理代码, 加入alternatives的语义，以及特征工程。
...
def TrainningsetProcess(dic_url, dataset_url, passage_len_limit):
    ...
    with open(dataset_url, 'r', encoding='utf-8') as ts_file:
        for file_line in ts_file:
            ...
            tmp_id = []
            for ans in tmp:
                if ans == None or ans == "<NULL>":
                    tmp_id.append([0])
                else:
                    ans = jieba.lcut(ans, cut_all=False)
                    if len(ans) > 5:
                        ans = ans[:5]
                        ans_over_limit += 1
                    tmp_id.append([dic.get(x) for x in ans])
            ...
            this_line_res['alternatives'] = tmp_id
            if dataset_url.find('test') == -1:
        ...
        print("ans_over_limit:{}".format(ans_over_limit))
...
def build_features(config, examples, data_type, out_file, is_test=False):
    ...
    list_nlp=[]
    with open("nlp_feature.json","r") as f:
        list_nlp=json.load(f)
    ...
    for example in tqdm(examples):
        ...
        alternative_idxs = np.zeros([3, ans_limit], dtype=np.int32)
        ...
        for i, token in enumerate(example["alternatives"]):
            for j, tk in enumerate(token):
                if tk == None:
                    alternative_idxs[i][j] = 0
                else:
                    alternative_idxs[i][j] = tk
        ...
        if not is_test:
            record = tf.train.Example(features=tf.train.Features(feature={
                                      ...
                                      "alternative_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[alternative_idxs.tostring()])),
                                      ...
                                      "nlp_feature":tf.train.Feature(float_list=tf.train.FloatList(value=list_nlp[int(example["query_id"])-1][1:]))
                                      }))
        else:
            record = tf.train.Example(features=tf.train.Features(feature={
                                      "passage_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[passage_idxs.tostring()])),
