...
data_process_aug.py：数据预处理代码(数据增强)
...
def build_features(config, examples, data_type, out_file, word2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        if data_type == "train":
            if example["answer"] == 0:
                ...
                depend_examples.append(example)
    ...
    # 数据增强，初步的增强是将不确定选项的答案搭配其他passage生成新的数据
    if data_type == "train":
        for example in depend_examples:
            random1 = random.randint(0, len(yes_examples) - 1)
            example["passage"] = yes_examples[random1]["passage"]
            total += 1
            passage_idxs = np.zeros([para_limit], dtype=np.int32)
            question_idxs = np.zeros([ques_limit], dtype=np.int32)
            for i, token in enumerate(example["passage"]):
            ...
            record = tf.train.Example(features=tf.train.Features(feature={
                                      ...
                                      "question_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[question_idxs.tostring()])),
                                      "answer": tf.train.Feature(int64_list=tf.train.Int64List(value=[example["answer"]])),
                                      "id": tf.train.Feature(int64_list=tf.train.Int64List(value=[example["query_id"] + 500000]))
                                      }))
            writer.write(record.SerializeToString())
        for example in depend_examples:
            random2 = random.randint(0, len(no_examples) - 1)
            example["passage"] = no_examples[random2]["passage"]
            total += 1
            passage_idxs = np.zeros([para_limit], dtype=np.int32)
            question_idxs = np.zeros([ques_limit], dtype=np.int32)
            for i, token in enumerate(example["passage"]):
            ...
            record = tf.train.Example(features=tf.train.Features(feature={
                                      ...
                                      "question_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[question_idxs.tostring()])),
                                      "answer": tf.train.Feature(int64_list=tf.train.Int64List(value=[example["answer"]])),
                                      "id": tf.train.Feature(int64_list=tf.train.Int64List(value=[example["query_id"] + 1000000]))
                                      }))
            writer.write(record.SerializeToString())
